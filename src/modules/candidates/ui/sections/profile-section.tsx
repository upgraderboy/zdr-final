...
export default function ProfileSectionSuspense() {
  const [candidateData] = trpc.candidates.getProfile.useSuspenseQuery();
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {resumeData?.hardSkills?.map((skill, index) => (
                          <Badge key={index} variant="secondary" className="text-sm" style={{ backgroundColor: resumeData?.colorHex ? resumeData?.colorHex : undefined }}>
                            {skill}
                          </Badge>
                        ))}
                      </div>
                ...
                <CardContent>
                  <div className="space-y-8">
                    {resumeData?.workExperiences?.map((experience, index) => (
                      ...
                      >
                        <div className="absolute -left-[9px] top-0 h-4 w-4 rounded-full" style={{ backgroundColor: resumeData?.colorHex ? resumeData?.colorHex : undefined }}></div>
                        <div className="space-y-2">
                          <h3 className="font-semibold text-lg" style={{ color: resumeData?.colorHex ? resumeData?.colorHex : undefined }}>{experience.position}</h3>
