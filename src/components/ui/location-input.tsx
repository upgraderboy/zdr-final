...
const LocationSelector = ({
  ...
  // Use refs to track if initialization has already occurred
const initializedCountry = React.useRef(false);
const initializedState = React.useRef(false);

useEffect(() => {
  if (!initializedCountry.current && initialCountryName) {
    const country = countriesData.find(c => c.name === initialCountryName);
    if (country) {
      setSelectedCountry(country);
      onCountryChange?.(country);
    }
    initializedCountry.current = true;
  }
}, [initialCountryName]);

useEffect(() => {
  if (!initializedState.current && selectedCountry && initialStateName) {
    const state = statesData.find(
      s => s.country_id === selectedCountry.id && s.name === initialStateName,
    );
    if (state) {
...
}, [initialStateName, selectedCountry]);
